...
class treestructure {
	...
	public function init() {
		...
		// --------------------------------------------- Mountpoints
		if(count($this->mountpoints = $REX["USER"]->getMountpoints()) && $this->category_id == 0)
		{
	...
	private function outputCategory($cat=null,$level=0)
	{
		...
		if(!empty($cat))
		{
			...
			if(!empty($buttons) || !empty($actions))
			{
				...
				// the id
				if(!empty($cat['id']))
					$return.=	'<span class="article-id">('.strval($cat['id']).')</span>';
	...
	private function outputArticle($article=null,$level=0)
	{
		...
		if(!empty($buttons) || !empty($actions))
		{
			...
			// the id
			$return.=	'<span class="article-id">('.strval($article['id']).')</span>';
	...
	public function walkCategories($cat=null)
	{
		...
		if(is_object($cat))
		{
			...
			$return['time']    		= date('d.m.Y H:i:s',$cat->getUpdateDate());
			...
			foreach($arts as $art)
			{
				if(!$art->isStartArticle())
				{
					$return['articles'][] = array(
						...
						'time'       => date('d.m.Y H:i:s',$art->getUpdateDate()),
...
/*
 		elseif ($function == 'artedit_function' && $data = rex_post('article','array',false))
		{
			// --------------------- ARTIKEL EDIT
			...
			$data['category_id'] = rex_post('category_id', 'rex-category-id');
			$data['path']        = rex_post('path', 'string', '');
...
*/		elseif ($this->reqfunction == 'artdelete_function' && $this->article_id != '' && $KATPERM && !$REX['USER']->hasPerm('editContentOnly[]'))
		...
		switch($func)
		{
			...
			case 'status' :  // change the status of an article
				...
				if(is_object($obj = OOArticle::getArticleById($data['article_id'])))
				{
					...
					if($REX['USER']->isAdmin())
						$valid = true;
					elseif($this->hasCatPerm($obj->getId()))
					{
	...
	public function processFunctionRequest($func=null) {
		...
		if(is_object($cat = OOCategory::getCategoryById($article['id'])))
			$article['path'] = $cat->getValue('path').$article['id'].'|';

		if(is_object($art = OOArticle::getArticleById($article['id'],$article['clang'])))
		{
			$article['name'] 		= $art->getName();
			$article['prior'] 		= $art->getPriority();
			$article['template_id']	= $art->getTemplateId();
		}
